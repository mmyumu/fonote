...
ignoring case and accents. A match whose details were not downloaded
stays reachable through its identifier; notes naming a match that no identifier can fetch
any more are not listed, since their card would open on nothing — they stay in the log. The calendar has a search within the displayed week,
...
- **The match fits in six cards side by side, with the pitch near the middle**, and no longer in a
  ...
  **faits** (facts), then the **statistiques**; to the left what I wrote — my **observations**,
  then the **bilan** (summary), then the **heatmap**. Each direction thus says what it brings back, the work stays in the
  ...
  status line. Swiping is the only way, and it is the same on all six cards; a bar
...
- Actions paired gesture by gesture, success above, failure below: good/bad, goal/own goal, pass/missed pass, dribble/failed dribble, shot on target/shot off target, won duel/lost duel, defensive action/lost ball, then yellow, red. Each carries a weight; polarity, colour and summary follow from it, instead of being picked by hand.
- **A cell may hold a family**, marked by a small corner: a tap gives the common gesture, a long press lays out the others above the cell, the way a keyboard offers its accents — slide onto one and lift, or lift first and tap. « Passe » holds « Passe D » (assist); « Tir » holds « Tête » (header on target), and « Tir M » its mirror, « Tête M »; « Défense » holds « Tacle » (tackle) and « Interception ». The cell shows the member the player was given, so that « Passe D » can be read at a glance; going back to a plain pass goes through the long press.
- **Save/missed save belong to the goalkeeper alone**, in a column of his own right after good/bad, which lead every palette; « Arrêt » also holds « Sortie » (coming off his line), and « Arrêt R » its mirror, « Sortie ratée ». The goalkeeper is recognised by his published position, or — a substitute the provider only calls "Substitute" — by the saves ESPN counts for goalkeepers alone. The tactical note's action list follows the same rule.
...
- **« Heatmap de mes notes »**, one swipe past the bilan: the bilan says how much, this card says where. One player at a time, picked in a row of chips — the most located first —, on a map the width of the card: a small map per player would have been a thumbnail where a handful of patches says nothing about which third they sit in. Each action is a soft patch, lime when it is good and vermilion when it is missed; overlapping patches show the zone the player keeps coming back to, and a success and a mistake on the same spot blend rather than one hiding the other. The two colours are the heat's own, the same under every theme, like the lawn they are painted on: a theme's green is picked for a page, and on grass it is green on green. The exact point sits on each patch — filled for a 📍, a ring for anything read off a board, only as exact as the drawing. The frame is the pitch's own, home attacking downwards, so a spot is where the finger put it; the head line says which way the player's team attacks.
  - A spot comes from three places (`Heat`). A quick note's **📍**. An action a **tactical note** gives a player, set where the board has him when he last had the ball — the pass leaves from there, the shot is struck there, the ball is lost there —, or where his last move ended if the ball never reached him. And what a board **draws without anyone saying it**: a ball that reaches a teammate is a good pass and one that reaches an opponent a missed one, both where it left the passer's feet — a ball played into space counts for whoever collects it, as long as nobody strikes it first —, and a tackle is a good defensive gesture. A shot says nothing either way, since the board does not know whether it went in, and neither does a run. Read-off actions only concern players the note gives no action — the given action is what was meant, and reading the drawing too would count the same pass twice —, and they never enter the bilan, which keeps to what was written.
  - An action said neither way is counted under the map (« 2 autres actions notées sans endroit »), and so are the players noted without any spot.
...
javac -cp "$FONOTE_JSON_JAR" -d /tmp/fonote-checks android/app/src/main/java/fr/fonote/{Track,Diagram,Sequence,TacticalHistory,Heat}.java android/checks/fr/fonote/{Track,Diagram,Sequence,Heat}Check.java
...
java -ea -cp "/tmp/fonote-checks:$FONOTE_JSON_JAR" fr.fonote.HeatCheck
